...
export const genRouteSlope = (lineSegment, userInput) => {
	...
	const numCoords = 200;
	// step 1: iter thru posXY to add Z, use distanceFromStart
	const coords = [];
	const distVec = [];
	const elevs = [];
	for ( let indexPass = 0; indexPass < stepCount; indexPass++ ) {
		...
		if (indexPass % 2 === 0) {
			for (let i = 0; i <= numCoords; i++) {
				...
				const startHiPass = startHi + indexPass * stepHi;
				const elev = utils.elevateFromDistance(
					dist,
					len,
					stepHi,
					startHiPass
			...
			feature.properties.elevation = elevs;
			feature.properties.elevMin = Math.min(...elevs);
			feature.properties.elevMax = Math.max(...elevs);
			feature.properties.passLen = len;
			feature.properties.distVec = distVec;
			...
			const distVecRev = [...distVec].reverse();
			const elevsRev = [...elevs].reverse();
			const geometryReverse = {
				type: "LineString",
				coordinates: coordsRev,
			};
			const featureRev = turf.feature(geometryReverse);
			featureRev.properties.elevation = elevs.map((d) => d + 1);
